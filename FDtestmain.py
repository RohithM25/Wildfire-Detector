...
def main():
    ...
    args = str(scale)+"-"+("%d" % percentageOrSet)+"-{}-{}-{}-{}".format(winWidth,winHeight,stepWidthp,stepHeightp)
    if drawBoxes: 
        dir = "./boxed images/"+args
        if not(os.path.exists(dir)):
            os.mkdir("./boxed images/"+args) #creating a directory for the boxed images with the parameters passed in by cmd line
    writeFile = open("./output/"+args+".txt",'w')
    ...
    def writeScaledBaseImagesToDisk():
        for i in range(len(baseImages)):
            imgName = baseImages[i][0]
            img = cv2.imread("./input images/HPWREN/"+imgName)
            scaled = cv2.resize(img,None, fx=scale, fy=scale)
            cv2.imwrite("./scaled images/scale={}-{}".format(scale,imgName),scaled)

    #writeScaledBaseImagesToDisk()
    ...
    for h in range(len(baseImages)):
        ...
        baseImage = cv2.resize(baseImage, None, fx=scale, fy=scale)
        ...
        params = fdparams.FDparams(writeFile,scale,scaledHeight,scaledWidth,windowWidth,windowHeight,stepWidth,stepHeight,args,pixelThreshold,drawBoxes)
